...
/* Api cliente */
  // Instancia cliente (determinación de baseUrl ya hace la lógica)
  const api = new APIClient();

  // Función para crear tarjeta HTML de producto
  function renderProductCard(p) {
    // Ajusta rutas si las imágenes están en una carpeta diferente
    const imgSrc = p.imagen || p.image || '../components/img/producto.webp';
    return `
      <article class="product-card" data-id="${p.id}">
        <img src="${imgSrc}" alt="${p.nombre || p.title}" />
        <h3>${p.nombre || p.title}</h3>
        <p class="price">${p.precio ? '$' + p.precio.toLocaleString() : ''}</p>
        <p class="category">${p.categoria || ''}</p>
        <p class="desc">${p.descripcion ? p.descripcion.substring(0,120) : ''}</p>
  ...
  async function loadProducts() {
    ...
    if (!container) return console.warn('No se encontró #productsList en la página.');

    // Petición al "mock backend"
    const res = await api.getProductos({ soloConStock: true });
    if (!res.success) {
